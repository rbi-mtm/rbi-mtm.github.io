...
/* LOCAL OVERRIDE: the vendor version closes the collapsed mobile menu on any
   <a> click that isn't exactly the ".dropdown-toggle" (the hamburger's
   sibling submenu openers). Since the theme toggle and search icon (see
   navbar.html) now live inside .navbar-collapse — moved there so they fold
   into the hamburger dropdown instead of overlapping the top bar on mobile —
   they're real <a> elements too, so flipping the theme switch or opening
   search used to also slam the whole mobile menu shut. Exclude both: they're
   controls, not navigation links, so closing the menu on click isn't the
...
$(document).on('click', '.navbar-collapse.show', function (e) {
  ...
  if (
    ...
    !targetElement.hasClass('theme-switch-hit') &&
    !targetElement.hasClass('js-search')
  ) {
    $(this).collapse('hide');
  }
});
